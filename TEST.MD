# 🧪 Full Stack Development Assessment (2 Hours)
...
i) req.query: Contains query string parameters from the URL. Example: /products?category=books -> req.query.category = 'books'
ii) req.params: Contains route parameters defined in the URL path. Example: /products/: id -> req.params.id = '123'
iii) req.body: Contains data sent in the request body, typically from POST/PUT requests

Q12:

i)Password hashing protects user credentials by converting passwords into irreversible values, ensuring they cannot be retrieved even if the database is compromised.
ii)Storing passwords in plain text is insecure because anyone accessing the database can read and misuse them, leading to account takeover and data breaches.

Q13:

Rate limiting is a technique used to control the number of requests a user can make to an API within a specific time frame. It helps prevent abuse, reduces server load, and protects against denial-of-service (DoS) attacks by limiting excessive requests from a single user or IP address.

Q14:

async/await allows asynchronous code to be written in a synchronous style, making it easier to read and maintain. In database operations, it enables waiting for a database query to complete before proceeding, ensuring that the application handles data correctly without blocking the event loop.

Q15:

findOne(): Retrieves a single document that matches the query criteria, returning the first match it finds.  find(): Retrieves all documents that match the query criteria, returning an array of results.
